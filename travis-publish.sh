...
if [[ $TRAVIS_REPO_SLUG == "grails/gorm-redis" && $TRAVIS_PULL_REQUEST == 'false' && $EXIT_STATUS -eq 0 ]]; then
  ...
  if [[ $EXIT_STATUS -eq 0 ]]; then
    ...
    if [[ -n $TRAVIS_TAG ]]; then
        ...
        version=${version:1}
         ...
         mkdir -p latest/redis
         cp -r ../docs/build/docs/. ./latest/redis/
         git add latest/redis/*

        majorVersion=${version:0:4}
        majorVersion="${majorVersion}x"

        mkdir -p "$version/redis"
        cp -r ../docs/build/docs/. "./$version/redis/"
        git add "$version/redis/*"

        mkdir -p "$majorVersion/redis"
        cp -r ../docs/build/docs/. "./$majorVersion/redis/"
        git add "$majorVersion/redis/*"

    else
        # If this is the master branch then update the snapshot
        mkdir -p snapshot/redis
        cp -r ../docs/build/docs/. ./snapshot/redis/

        git add snapshot/redis/*
    fi


    git commit -a -m "Updating Redis Docs for Travis build: https://travis-ci.org/$TRAVIS_REPO_SLUG/builds/$TRAVIS_BUILD_ID"
